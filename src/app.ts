...
export class App {
    ...
    const handling = new Promise<void>(async (resolve, _) => {
      try {
        if (handler.handleComponent) {
          ...
          this.logger.error(`No component handlers found`, { interaction });
          await context.edit({
        ...
        this.logger.error(`Error occured during interaction`, {
          error,
          interaction,
        });
      }
  ...
  async #handleApplicationCommand(interaction: APIApplicationCommandInteraction): Promise<APIInteractionResponse> {
    ...
    if (!handler) {
      this.logger.error(`No handlers found for command: ${interaction.data.name}`, { interaction });
      return {
        type: InteractionResponseType.ChannelMessageWithSource,
        data: {
          flags: MessageFlags.Ephemeral,
